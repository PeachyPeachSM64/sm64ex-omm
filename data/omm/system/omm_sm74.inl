...
static void omm_opt_init_warp_to_level() {
    for (sWarpDest.areaIdx = 1; sWarpDest.areaIdx <= 2; ++sWarpDest.areaIdx) {
        for (s32 i = 0; i != omm_level_get_count(); ++i) {
            ...
            for (s32 j = 0; j != 6; ++j) {
                if ((stars >> j) & 1) {
                    ...
                    actValues->act = j + 1;
                    omm_array_add(sOmmWarpActValues[sWarpDest.areaIdx], ptr, actValues);
                }
            }
        }
    }
    ...
    for (sWarpDest.areaIdx = 1; sWarpDest.areaIdx <= 2; ++sWarpDest.areaIdx) {
        for (s32 i = 0; i != omm_level_get_count(); ++i) {
            ...
            sOmmWarpLevelChoices[sWarpDest.areaIdx][i] = omm_dup(name, omm_text_length(name) + 1);
        }
    }
    ...
    for (sWarpDest.areaIdx = 1; sWarpDest.areaIdx <= 2; ++sWarpDest.areaIdx) {
        omm_array_for_each(sOmmWarpActValues[sWarpDest.areaIdx], p) {
            ActValues *actValues = (ActValues *) p->as_ptr;
            const u8 *name = omm_level_get_act_name(actValues->level, actValues->act, true, true);
            sOmmWarpActChoices[sWarpDest.areaIdx][i_p] = omm_dup(name, omm_text_length(name) + 1);
        }
    }
}
