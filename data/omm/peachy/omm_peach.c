...
s32 omm_act_peach_glide(struct MarioState *m) {
    if (m->actionState == 0) {
        ...
        ANM(MARIO_ANIM_GRAB_POLE_SHORT, 2.f);
        m->actionState = 1;
    }
    ...
    // Gfx
    if (++m->actionTimer > 6) ANM(MARIO_ANIM_IDLE_ON_POLE, 1.f);
    return OMM_MARIO_ACTION_RESULT_CONTINUE;
}
...
s32 omm_act_peach_attack_ground(struct MarioState *m) {
    ...
    switch (m->actionArg) {
        ...
        case 4: {
            ...
            if (OMM_EXTRAS_SMO_ANIMATIONS) {
                ...
                m->actionState = 1 + (m->marioObj->oAnimFrame <= 20);
            } else {
    ...
    // Gfx
    PFX(PARTICLE_DUST * (abs_f(m->forwardVel) > 16.f));
...
s32 omm_act_peach_attack_fast(struct MarioState *m) {
    ...
    if (m->actionTimer++ == 0 && OMM_PEACH_SPAWN_PERRY_SHOCKWAVE) {
        omm_obj_spawn_perry_shockwave(m->marioObj, 4, omm_perry_get_type(m), false);
    }
    ...
    switch (m->actionArg) {
        ...
        // First attack (part 1)
        case 0: {
            if (m->actionState++ == 1) omm_peach_play_random_attack_sound(m);
            ANM(MARIO_ANIM_FIRST_PUNCH, 1.f);
            action_condition(obj_anim_is_near_end(m->marioObj), 0, 0, NO_RETURN, m->actionArg = 1; m->actionState = 1;);
        } break;

        // First attack (part 2)
        case 1: {
            ANM(MARIO_ANIM_FIRST_PUNCH_FAST, 1.f);
            action_condition(++m->actionState > 4, 0, 0, NO_RETURN, m->actionArg = 2; m->actionState = 1;);
        } break;
        ...
        case 2: {
            ...
            action_condition(obj_anim_is_near_end(m->marioObj), 0, 0, NO_RETURN, m->actionArg = 3; m->actionState = 1;);
        } break;

        // Second attack (part 2)
        case 3: {
            ANM(MARIO_ANIM_SECOND_PUNCH_FAST, 1.f);
            action_condition(++m->actionState > 4, ACT_WALKING, 0, RETURN_BREAK);
        } break;
    }
...
s32 omm_act_peach_attack_air(struct MarioState *m) {
    ...
    // Gfx
    if (OMM_EXTRAS_SMO_ANIMATIONS) {
        ANM(MARIO_ANIM_OMM_CAPPY_THROW, 1.5f);
        obj_anim_clamp_frame(m->marioObj, 31, 127);
        m->actionArg = (m->marioObj->oAnimFrame < 52);
    } else {
        ANM(MARIO_ANIM_FINAL_BOWSER_RAISE_HAND_SPIN, 2.f);
        obj_anim_clamp_frame(m->marioObj, 70, 94);
        m->actionArg = (m->marioObj->oAnimFrame < 94);
    }
    m->marioBodyState->punchState = 0;
    return OMM_MARIO_ACTION_RESULT_CONTINUE;
}
...
s32 omm_act_peach_perry_charge_air(struct MarioState *m) {
    if (m->actionArg) { m->actionState = m->actionArg; m->actionArg = 0; }
    action_init_no_y_vel(m->forwardVel, 0, SOUND_MARIO_UH2);
...
s32 omm_act_peach_vibe_joy_fly(struct MarioState *m) {
    action_init_no_y_vel(m->forwardVel, 0, SOUND_ACTION_INTRO_UNK45E);
    action_condition(!omm_peach_vibe_is_joy(), ACT_FREEFALL, 0, RETURN_CANCEL);
...
s32 omm_act_peach_vibe_joy_attack(struct MarioState *m) {
    action_init(m->forwardVel, max_f(m->vel[1], 10.f), 0, SOUND_ACTION_FLYING_FAST, omm_obj_spawn_peach_vibe_joy_gust(m->marioObj, 60.f, m->faceAngle[1]););
    action_condition(!omm_peach_vibe_is_joy(), ACT_FREEFALL, 0, RETURN_CANCEL);
...
static const PerryChargeAction OMM_PERRY_CHARGE_ACTIONS[] = {
    { 0,                                 0, 0, 0, 0 },
...
static const PerryChargeAction *omm_act_peach_perry_charge_get_action(struct MarioState *m) {
    for (s32 i = 0; i != (s32) array_length(OMM_PERRY_CHARGE_ACTIONS); ++i) {
        if (OMM_PERRY_CHARGE_ACTIONS[i].action == m->action) {
            return &OMM_PERRY_CHARGE_ACTIONS[i];
        }
    }
    return &OMM_PERRY_CHARGE_ACTIONS[0];
}
...
OMM_ROUTINE_PRE_RENDER(omm_act_peach_perry_charge_update) {
    ...
    if (gMarioObject && !omm_is_game_paused() && !(gTimeStopState & TIME_STOP_ENABLED)) {
        ...
        const PerryChargeAction *a = omm_act_peach_perry_charge_get_action(m);
