...
// Surface groups
#define SURFACE_IS_QUICKSAND(type)              ((type) == SURFACE_INSTANT_MOVING_QUICKSAND || ((type) >= SURFACE_SHALLOW_QUICKSAND && (type) <= SURFACE_MOVING_QUICKSAND))
#define SURFACE_IS_NOT_HARD(type)               ((type) != SURFACE_HARD && !((type) >= SURFACE_HARD_SLIPPERY && (type) <= SURFACE_HARD_NOT_SLIPPERY))
#define SURFACE_IS_PAINTING_WARP(type)          ((type) >= SURFACE_PAINTING_WARP_D3 && (type) <= SURFACE_PAINTING_WARP_FC)
#define SURFACE_IS_DEATH_PLANE(type)            ((type) == SURFACE_DEATH_PLANE || (type) == SURFACE_VERTICAL_WIND)
#define SURFACE_IS_LETHAL(type)                 ((type) == SURFACE_DEATH_PLANE || (type) == SURFACE_VERTICAL_WIND || (type) == SURFACE_BURNING)
...
#define SURFACE_CEIL                            1
#define SURFACE_WALL                            2
#define SURFACE_CATEGORY(y)                     (((y) > +0.01f) ? SURFACE_FLOOR : (((y) < -0.01f) ? SURFACE_CEIL : SURFACE_WALL))
...
// Collision commands
#define COL_CMD_INIT                            0x0000
#define COL_CMD_VERTEX_INIT                     0x0001
#define COL_CMD_VERTEX                          0x0002
#define COL_CMD_TRI_INIT                        0x0003
#define COL_CMD_TRI                             0x0004
#define COL_CMD_TRI_SPECIAL                     0x0005
#define COL_CMD_TRI_STOP                        0x0006
#define COL_CMD_END                             0x0007
#define COL_CMD_SPECIAL_INIT                    0x0008
#define COL_CMD_WATER_BOX_INIT                  0x0009
#define COL_CMD_TRANSLATE                       0x000A
#define COL_CMD_JUMP                            0x000B
#define COL_CMD_END_BYTE_SEQUENCE               0x8708, 0x9749, 0x6E13, 0xA3B6, 0x83F8, 0x7245, 0x575D, 0xB1AA
#define COL_INIT()                              COL_CMD_INIT
#define COL_VERTEX_INIT(vtxNum)                 COL_CMD_VERTEX_INIT, vtxNum
#define COL_VERTEX(x, y, z)                     COL_CMD_VERTEX, x, y, z
#define COL_TRI_INIT(surfType, triNum)          COL_CMD_TRI_INIT, surfType, triNum
#define COL_TRI(v1, v2, v3)                     COL_CMD_TRI, v1, v2, v3
#define COL_TRI_SPECIAL(v1, v2, v3, param)      COL_CMD_TRI_SPECIAL, v1, v2, v3, param
#define COL_TRI_STOP()                          COL_CMD_TRI_STOP
#define COL_END()                               COL_CMD_END, COL_CMD_END_BYTE_SEQUENCE
#define COL_SPECIAL_INIT(num)                   COL_CMD_SPECIAL_INIT, num
#define COL_WATER_BOX_INIT(num)                 COL_CMD_WATER_BOX_INIT, num
#define COL_WATER_BOX(id, x1, z1, x2, z2, y)    id, x1, z1, x2, z2, y
#define COL_TRANSLATE(x, y, z)                  COL_CMD_TRANSLATE, x, y, z
#define COL_JUMP(index)                         COL_CMD_JUMP, index
